...
public class RatingRepository {
    ...
    public void updateRatingsAverage(Movie movie){
        try (Connection conn = dataSource.getConnection();
             PreparedStatement stmt = conn.prepareStatement("UPDATE movies SET average_rating=(SELECT AVG(rating) FROM ratings WHERE movie_id=?) WHERE id=?;");
        ) {
            stmt.setInt(1, movie.getId());
            stmt.setInt(2, movie.getId());
            stmt.execute();
        } catch (SQLException sqle) {
            throw new IllegalStateException("Can not update average after ratings insert!", sqle);
        }
    }
